...
def get_qa_chain(k: int = 5, temperature: float = 0.0):
    """
    ...
    system_template = """\
        ...
        回答では、根拠としたルール名があれば明示してください。例：「競技規定によると、」「競技会規定によると、」のように、該当するルール名を示してください。複数のルールにまたがる場合はそれぞれ言及してください。
        """
    human_template = """\
        質問:
        {question}
        ...
        上記のコンテキスト以外の情報は一切使わずに答えてください。また、根拠としたルール名（競技規定・競技細則・競技会規定など）があれば明示してください。
        """
